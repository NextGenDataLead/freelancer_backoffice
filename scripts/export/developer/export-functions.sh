...
# Load environment variables if .env.export exists
load_env_vars() {
    local env_file="${BASH_SOURCE%/*}/.env.export"
    if [[ -f "$env_file" ]]; then
        log "Loading environment variables from .env.export"
...
check_supabase_cli() {
    local base_dir="$(dirname "$0")"
    local local_cli="${base_dir}/node_modules/supabase/bin/supabase"
    
    # Try local CLI first, then global
    if [[ -f "$local_cli" ]]; then
        SUPABASE_CLI="$local_cli"
        local cli_version
        cli_version=$($SUPABASE_CLI --version 2>/dev/null | head -n1)
        success "Found local Supabase CLI: $cli_version"
    elif command -v supabase &> /dev/null; then
        SUPABASE_CLI="supabase"
        local cli_version
        cli_version=$($SUPABASE_CLI --version 2>/dev/null | head -n1)
        success "Found global Supabase CLI: $cli_version"
    else
...
list_functions() {
    ...
    if functions_response=$(curl -s \
        ...
        if [[ "$functions_response" == "[]" ]]; then
            success "No Edge Functions found in project"
            echo ""
        elif function_names=$(echo "$functions_response" | jq -r '.[].name' 2>/dev/null); then
