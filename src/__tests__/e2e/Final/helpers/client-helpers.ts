...
export async function createClientViaAPI(
  ...
  const baseEmail = data.email || `${data.name.toLowerCase().replace(/\s+/g, '.')}@example.com`
  console.log(`[CLIENT-HELPER] Attempting to create client: ${data.name}`)

  const payload = {
    company_name: data.name,
    email: baseEmail,
    phone: data.phone || '+31612345678',
  ...
  if (!response.ok()) {
    ...
    console.error(`[CLIENT-HELPER] Failed to create client: ${data.name}. Status ${response.status()}, body: ${error}`)
    throw new Error(`Failed to create client via API: ${response.status()} - ${error}`)
  }

  const result = await response.json()
  const clientId = result.client?.id || result.data?.id || result.id
  console.log(`[CLIENT-HELPER] Created client ${data.name} with id ${clientId}`)
  return clientId
}
