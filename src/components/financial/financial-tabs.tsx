...
const OverviewTabContent = ({ onTabChange }: { onTabChange: (tabId: string) => void }) => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
      ...
      {/* Sidebar - Quick Actions for Financial Features */}
  ...
  const tabs: FinancialTab[] = [
    {
      ...
      content: <OverviewTabContent onTabChange={handleTabChange} />
    },
    {
      id: 'tijd',
      label: 'Time Tracking',
  ...
  return (
    <div className={className}>
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        {/* Tab Navigation */}
        <TabsList className="grid w-full grid-cols-3 sm:grid-cols-6 lg:grid-cols-6 h-auto p-1 bg-muted/50">
          {tabs.map((tab) => (
            <TabsTrigger
              key={tab.id}
              value={tab.id}
