...
import { Clock, Plus, ArrowLeft, Play, Pause, Square, Euro, Users, Keyboard, List, Calendar as CalendarIcon } from 'lucide-react'
...
export default function TimeTrackingPage() {
  ...
  const handleTimeEntryCreated = (timeEntry: any) => {
    ...
    fetchClients()
    // Refresh stats to reflect new time entry
    fetchTimeStats()
  }
  ...
  const handleTimeEntryUpdated = (timeEntry: any) => {
    ...
    setRefreshKey(prev => prev + 1)
    // Refresh stats to reflect updated time entry
    fetchTimeStats()
  }

  const handleRefresh = () => {
    setRefreshKey(prev => prev + 1)
    fetchTimeStats() // Also refresh stats when refreshing data
  }
    ...
    if (elapsed > 0.0001 && selectedClientId) { // More precise minimum threshold
      ...
      try {
        const timeEntryData = {
          ...
          project_name: selectedProject || '', // Keep for backwards compatibility
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <UnifiedTimeEntryDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) {
            // Reset calendar mode when dialog closes
            setCalendarMode(false)
            setSelectedCalendarDate(undefined)
          }
        }}
