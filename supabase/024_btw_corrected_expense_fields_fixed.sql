-- Migration: CORRECTED BTW expense fields for official Dutch VAT form compliance (FIXED)
...
-- FIXED: Works with existing expenses table structure
...
-- Update existing expenses with corrected structure (FIXED to work with actual schema)
UPDATE expenses SET 
    acquisition_type = CASE
        ...
        WHEN supplier_country_code IN ('BE', 'DE', 'FR', 'ES', 'IT', 'PL', 'SE', 'DK', 'FI', 'AT', 'PT', 'IE', 'LU', 'EE', 'LV', 'LT', 'SI', 'SK', 'CZ', 'HU', 'RO', 'BG', 'HR', 'CY', 'MT', 'GR') THEN 
            -- For EU countries, assume services unless we can determine goods
            CASE WHEN expense_type = 'travel' OR expense_type = 'equipment' THEN 'eu_goods' ELSE 'eu_services' END
        ELSE 'import_goods'
    END,
