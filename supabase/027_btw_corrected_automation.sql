-- Migration: CORRECTED BTW automation triggers and data synchronization
    ...
    ELSIF TG_TABLE_NAME = 'expenses' THEN
        IF NEW.acquisition_type IN ('eu_goods', 'eu_services', 'import_goods', 'import_services') THEN
            INSERT INTO international_trade_transactions (
                ...
                NEW.description, -- Use description as supplier name
                NEW.amount,
...
-- Function to recalculate all BTW forms for a period
...
BEGIN
    -- Process specific quarter or all quarters
    FOR quarter_to_process IN 
        SELECT CASE WHEN p_quarter IS NULL THEN generate_series(1, 4) ELSE p_quarter END
    LOOP
        -- Ensure BTW form exists
        INSERT INTO quarterly_btw_forms (tenant_id, year, quarter)
...
-- Sync existing data to new structure
DO $$
DECLARE
    tenant_record RECORD;
BEGIN
    -- Process each tenant
    FOR tenant_record IN SELECT DISTINCT tenant_id FROM invoices LOOP
        -- Recalculate current year
        PERFORM recalculate_btw_forms_for_period(
            tenant_record.tenant_id, 
            EXTRACT(year FROM CURRENT_DATE)::integer
        );
        
        -- Recalculate previous year if we're in Q1
        IF EXTRACT(quarter FROM CURRENT_DATE) = 1 THEN
            PERFORM recalculate_btw_forms_for_period(
                tenant_record.tenant_id, 
...
COMMENT ON FUNCTION recalculate_btw_forms_for_period IS 'Recalculate BTW forms using CORRECTED calculations for specified period';
