-- Migration: CORRECTED BTW expense fields for official Dutch VAT form compliance (FIXED v3)
...
-- FIXED v3: Adds unique constraint for ON CONFLICT and uses actual enum values
...
ADD COLUMN IF NOT EXISTS business_use_percentage_corrected NUMERIC(5,2) DEFAULT 100 CHECK (business_use_percentage_corrected >= 0 AND business_use_percentage_corrected <= 100),
...
ADD COLUMN IF NOT EXISTS supplier_country VARCHAR(2) DEFAULT 'NL',
ADD COLUMN IF NOT EXISTS supplier_vat_number_corrected VARCHAR(50),
...
COMMENT ON COLUMN expenses.business_use_percentage_corrected IS 'Percentage of business use (affects VAT deductibility) - corrected version';
...
-- Create expense VAT calculation details table for audit trail (FIXED with unique constraint)
CREATE TABLE IF NOT EXISTS expense_vat_calculation (
    ...
    -- FIXED: Add unique constraint for ON CONFLICT
    UNIQUE(expense_id)
);
...
CREATE INDEX IF NOT EXISTS idx_expenses_supplier_country_corrected ON expenses(tenant_id, supplier_country) WHERE supplier_country != 'NL';
...
-- CORRECTED function to calculate expense VAT for Section 5b (FIXED - no ON CONFLICT issues)
...
BEGIN
    -- Use corrected business percentage or fall back to existing one
    effective_business_pct := COALESCE(NEW.business_use_percentage_corrected, NEW.business_percentage, 100) - COALESCE(NEW.private_use_percentage, 0);
    IF effective_business_pct < 0 THEN
        effective_business_pct := 0;
    END IF;
    
    -- Calculate VAT amounts based on expense type
    IF NEW.acquisition_type = 'domestic' OR NEW.acquisition_type IS NULL THEN
    ...
    ELSIF NEW.acquisition_type IN ('import_goods', 'import_services') THEN
        ...
        gross_vat := COALESCE(NEW.import_vat_paid, 0);
        deductible_vat := gross_vat * effective_business_pct / 100;
    ...
    -- Update existing fields for consistency if they exist
    IF gross_vat > 0 THEN
        NEW.vat_amount := gross_vat;
        NEW.is_vat_deductible := (deductible_vat > 0);
    END IF;
    
    -- Handle calculation record (FIXED - check for existing record first)
    SELECT id INTO existing_calc_id FROM expense_vat_calculation WHERE expense_id = NEW.id;
    
    IF existing_calc_id IS NOT NULL THEN
        -- Update existing record
        UPDATE expense_vat_calculation SET
            gross_amount = NEW.amount + gross_vat,
            net_amount = NEW.amount,
            vat_rate = COALESCE(NEW.vat_rate, 0),
            business_percentage = effective_business_pct,
            gross_vat_amount = gross_vat,
            deductible_vat_amount = deductible_vat,
            non_deductible_vat_amount = non_deductible_vat,
            contributes_to_section_5b = (deductible_vat > 0),
            deduction_limitation = NEW.deduction_limitation_reason,
            reverse_charge_owed = COALESCE(NEW.reverse_charge_received, 0),
            reverse_charge_deductible = COALESCE(NEW.reverse_charge_deductible, 0),
            calculation_method = 'corrected_calculation_v3_update',
            calculation_date = NOW()
        WHERE id = existing_calc_id;
    ELSE
        -- Insert new record
        INSERT INTO expense_vat_calculation (
            ...
            'corrected_calculation_v3_insert'
        );
    END IF;
...
CREATE TRIGGER calculate_corrected_expense_vat_trigger
    BEFORE INSERT OR UPDATE OF amount, vat_rate, vat_amount, business_use_percentage_corrected, private_use_percentage, 
...
-- Update existing expenses with corrected structure (FIXED to use actual enum values)
UPDATE expenses SET 
    acquisition_type = CASE
        WHEN supplier_country_code = 'NL' OR supplier_country_code IS NULL THEN 'domestic'
        WHEN supplier_country_code IN ('BE', 'DE', 'FR', 'ES', 'IT', 'PL', 'SE', 'DK', 'FI', 'AT', 'PT', 'IE', 'LU', 'EE', 'LV', 'LT', 'SI', 'SK', 'CZ', 'HU', 'RO', 'BG', 'HR', 'CY', 'MT', 'GR') THEN 
            -- For EU countries, use actual enum values: office_supplies might be goods, others typically services
            CASE WHEN expense_type IN ('office_supplies') THEN 'eu_goods' ELSE 'eu_services' END
        ELSE 'import_goods'
    END,
    supplier_country = COALESCE(supplier_country_code, 'NL'),
    business_use_percentage_corrected = COALESCE(business_percentage, 100),
    private_use_percentage = 0,
    supplier_vat_number_corrected = supplier_vat_number,
